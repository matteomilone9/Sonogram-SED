...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        audio_path = os.path.normpath(audio_path)  # converte / e \ nel formato corretto per il sistema
        ...
        waveform = waveform.squeeze(0)  # [len]

        if self.supervised:
            label = int(self.labels[idx])
            return waveform, label,  torch.tensor(0), torch.tensor(0)
        else:
            return waveform
